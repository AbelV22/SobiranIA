...
export function ComparisonSection() {
  ...
  return (
    <section id="comparativa" className="relative py-24 md:py-32 lg:py-40 bg-[#FAFAFA]">
      <div ref={ref} className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white border border-black/[0.06] mb-6 apple-shadow">
            ...
            <span className="text-sm font-medium text-[#0071E3]">Comparativa</span>
          </div>
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-[#1D1D1F] mb-6">
            Núvol vs <span className="text-gradient">Local</span>
          </h2>
        ...
        <div className="flex md:hidden justify-center gap-2 mb-8">
          {[
            ...
            { key: 'side' as const, label: 'Ambdós', color: 'text-[#1D1D1F]' },
            { key: 'local' as const, label: 'Local', color: 'text-[#0071E3]' },
          ].map((opt) => (
            <button
              key={opt.key}
              onClick={() => setView(opt.key)}
              className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-200 ${
                view === opt.key ? `${opt.color} bg-black/5` : 'text-[#86868B]'
              }`}
            >
              {opt.label}
            </button>
        ...
        >
          <div className="hidden md:grid grid-cols-[1fr_1fr_1fr] gap-4 mb-6">
            <div />
            <div className="text-center p-4 rounded-2xl bg-red-50/50 border border-red-100">
              <div className="flex items-center justify-center gap-2 mb-3">
                <Cloud className="w-5 h-5 text-red-500" />
                <span className="font-semibold text-red-500">IA al Núvol</span>
              </div>
              <RiskMeter animated={isVisible} />
            </div>
            <div className="text-center p-4 rounded-2xl bg-[#0071E3]/[0.03] border border-[#0071E3]/10">
              <div className="flex items-center justify-center gap-2 mb-3">
                <Server className="w-5 h-5 text-[#0071E3]" />
                <span className="font-semibold text-[#0071E3]">IA Local</span>
              </div>
          ...
          <div className="space-y-3">
            {comparisons.map((row, index) => (
              <div
                key={index}
                className={`rounded-2xl bg-white apple-shadow overflow-hidden transition-all duration-500 hover:apple-shadow-lg ${
              ...
              >
                ...
                >
                  <div className="w-10 h-10 rounded-xl bg-[#0071E3]/[0.06] flex items-center justify-center flex-shrink-0">
                    <row.icon className="w-5 h-5 text-[#0071E3]" />
                  </div>

                  <div className="flex-1 min-w-0">
                    <div className="font-semibold text-[#1D1D1F] mb-2">{row.label}</div>
                    <div className="hidden md:grid grid-cols-2 gap-6">
                      <ScoreBar score={row.cloud.score} color="#EF4444" animated={isVisible} />
                      <ScoreBar score={row.local.score} color="#0071E3" animated={isVisible} />
                    </div>
                    <div className="md:hidden space-y-2">
                      {(view === 'side' || view === 'cloud') && (
                        <div>
                          <div className="text-xs text-red-500 mb-1 font-medium">Núvol</div>
                          <ScoreBar score={row.cloud.score} color="#EF4444" animated={isVisible} />
                        </div>
                      )}
                      {(view === 'side' || view === 'local') && (
                        <div>
                          <div className="text-xs text-[#0071E3] mb-1 font-medium">Local</div>
                          <ScoreBar score={row.local.score} color="#0071E3" animated={isVisible} />
                        </div>
                      )}
                    </div>
                  ...
                  <ChevronDown
                    className={`w-5 h-5 text-[#86868B] transition-transform duration-300 flex-shrink-0 ${
                ...
                <div
                  className={`overflow-hidden transition-all duration-300 ${
                    expandedRow === index ? 'max-h-40 opacity-100' : 'max-h-0 opacity-0'
                  }`}
                >
                  <div className="px-4 md:px-5 pb-4 md:pb-5 grid md:grid-cols-2 gap-4">
                    <div className="flex items-start gap-2 p-3 rounded-xl bg-red-50/50">
                      <X className="w-4 h-4 text-red-500 mt-0.5 flex-shrink-0" />
                      <span className="text-sm text-[#6E6E73]">{row.cloud.text}</span>
                    </div>
                    <div className="flex items-start gap-2 p-3 rounded-xl bg-[#0071E3]/[0.04]">
                      <Check className="w-4 h-4 text-[#0071E3] mt-0.5 flex-shrink-0" />
                      <span className="text-sm text-[#6E6E73]">{row.local.text}</span>
                    </div>
                  </div>
                </div>
              </div>
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white border border-black/[0.06] apple-shadow">
            <AlertTriangle className="w-4 h-4 text-[#0071E3]" />
            <span className="text-sm text-[#86868B]">
              El <span className="text-[#0071E3] font-semibold">92% de les empreses</span> que proven IA local no tornen al núvol
            </span>
          </div>
        </div>
      </div>
